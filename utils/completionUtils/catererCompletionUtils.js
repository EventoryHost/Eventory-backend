...
export const checkCatererProfileCompletion = async (catererId) => {
    try {
        console.log(`Checking profile completion for caterer with ID: ${catererId}`);
        ...
        await Caterer.findOneAndUpdate({ id: catererId }, {
        ...
        const eventDetailsComplete = caterer.eventDetails.event_types_catered.length > 0 && caterer.eventDetails.additional_services.length > 0;
        ...
        const staffAndEquipmentComplete = caterer.staffAndEquipmentDetails.staff_provided.length > 0 && caterer.staffAndEquipmentDetails.equipment_provided.length > 0;
        console.log(`Staff and Equipment Details Complete: ${staffAndEquipmentComplete}`);
        ...
        if (caterer.policies && caterer.policies.terms_and_conditions && caterer.policies.terms_and_conditions.trim() !== '') {
            termsComplete = true;
        }
        ...
        if (caterer.policies && caterer.policies.client_testimonials && caterer.policies.client_testimonials.trim() !== '') {
        ...
        if (caterer.policies && caterer.policies.cancellation_policy && caterer.policies.cancellation_policy.trim() !== '') {
