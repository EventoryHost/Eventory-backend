...
router.post("/", async (req, res) => {
    ...
    try {
        ...
        if (existingDetails) {
            ...
            return res.status(200).json({ message: "Gift details updated successfully.", data: updatedDetails });
        } else {
            const newGiftDetails = new GiftModel({ userId, ...giftsData });
            await newGiftDetails.save();
            return res.status(201).json({ message: "Gift details saved successfully.", data: newGiftDetails });
        }
    } catch (error) {
        console.error("Error saving/updating gift details:", error);
        res.status(500).json({ message: "Failed to save or update gift details.", error: error.message });
    }
});
...
router.get("/:userId", async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({ message: "Failed to retrieve gift details.", error: error.message });
    }
});
