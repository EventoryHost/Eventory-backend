...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    const message = updatedDetails.isNew
      ? "Decorator details saved successfully."
      : "Decorator details updated successfully.";
...
router.get("/:vendor_id", async (req, res) => {
  ...
  try {
    const decoratorDetails = await ReduxDecoratorModel.findOne({
...
router.delete("/:vendor_id", async (req, res) => {
  ...
  try {
    const deletedDetails = await ReduxDecoratorModel.findOneAndDelete({
      vendor_id: vendor_id,
    });
