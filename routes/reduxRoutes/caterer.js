...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    const message = updatedDetails.isNew
      ...
      : "Catering details updated successfully.";
...
router.get("/:vendor_id", async (req, res) => {
  ...
  try {
    const cateringDetails = await ReduxCatererModel.findOne({
...
router.delete("/:vendor_id", async (req, res) => {
  ...
  try {
    const deletedDetails = await ReduxCatererModel.findOneAndDelete({
      vendor_id: vendor_id,
    });
