...
router.post("/", async (req, res) => {
    ...
    if (!pavData || Object.keys(pavData).length === 0) {
      return res.status(400).json({ message: "photographer details are required." });
    }
    ...
    try {
        ...
        const updatedDetails = await ReduxPhotographerVideographerModel.findOneAndUpdate(
          { vendor_id },
          dataToSave,
          { new: true, upsert: true }
        );
        
        const message = updatedDetails.isNew ? "photographer details saved successfully." : "photographer details updated successfully.";
...
router.get("/:vendor_id", async (req, res) => {
    ...
    try {
      const pavDetails = await ReduxPhotographerVideographerModel.findOne({ vendor_id: vendor_id.trim() });
  
      if (!pavDetails) {
        return res.status(404).json({ message: "photographer details not found." });
      }
...
router.delete("/:vendor_id", async (req, res) => {
    ...
    try {
      const deletedDetails = await ReduxPhotographerVideographerModel.findOneAndDelete({ vendor_id: vendor_id }); 
  
      if (!deletedDetails) {
        return res.status(404).json({ message: "photographer details not found for deletion." });
      }
  
      res.status(200).json({ message: "photographer details deleted successfully." });
