...
const normalizeType = (t = "") => {
  ...
  if (
    ["venue", "venue-provider", "venue provider", "venue_provider"].includes(t)
  )
  ...
  if (["dj", "dj-artist", "dj artist", "dj_artist", "djartists"].includes(t))
    return "djartists"; // OK
  return t;
};
...
export const createReview = async (req, res) => {
  try {
    ...
    const customer_name =
      body.customer_name || body.reviewerName || body.customerName;
    const service_type = normalizeType(
      body.service_type || body.vendorType || body.type,
    );
    const rating =
      typeof body.rating === "number" ? body.rating : parseFloat(body.rating);
    const review = body.review || body.feedback || "";
    const media_photo =
      body.media_photo ||
      (Array.isArray(body.photos) ? body.photos[0] : body.photo) ||
      "";
    ...
    if (
      ...
      return res.status(400).json({
        ...
        required:
          "service_id, customer_id, customer_name, service_type, rating",
      });
    }
...
export const getReviewsByVendor = async (req, res) => {
  try {
    ...
    const reviews = await Reviews.find({ service_id }).sort({
      feedback_submitted_at: -1,
    });
