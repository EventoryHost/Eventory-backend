...
export const createBooking = async (req, res) => {
  try {
    console.log(
      "[BOOKING_DEBUG] createBooking triggered with body:",
    ...
    if (typeof final_amount === "undefined" || final_amount === null)
      missing.push("final_amount");
    ...
    if (locType && !["INDOOR", "OUTDOOR"].includes(locType)) {
      return res
        .status(400)
        .json({ message: "location_type must be INDOOR or OUTDOOR" });
    }

    const statusNorm = event_status ? event_status.toLowerCase() : undefined;
    if (
      statusNorm &&
      !["booked", "upcoming", "ongoing", "completed", "cancelled"].includes(
        statusNorm,
      )
    ) {
    ...
    const payStatusNorm = payment_status
      ? payment_status.toLowerCase()
      : undefined;
    if (
      payStatusNorm &&
      !["advance_paid", "fully_paid", "refunded"].includes(payStatusNorm)
    ) {
    ...
    if (!(eventStartDate instanceof Date) || isNaN(eventStartDate)) {
      return res
        .status(400)
        .json({ message: "event_start must be a valid date" });
    }
    if (
      (eventEndDate && !(eventEndDate instanceof Date)) ||
      (eventEndDate && eventEndDate < eventStartDate)
    ) {
      console.log(
        `[BOOKING_DEBUG] Validation failed: eventEndDate(${eventEndDate}) < eventStartDate(${eventStartDate})`,
      );
      return res
        .status(400)
        .json({ message: "event_end must be after or equal to event_start" });
    }
    ...
    if (alreadyPaid > finalAmountNum) {
      return res
        .status(400)
        .json({ message: "already_paid_amount cannot exceed final_amount" });
    }
    ...
    const payment_details = paymentDetails
      ? {
          customerPayable: {
            ...
            baseAmount: asNumber(
              paymentDetails?.customerPayable?.baseAmount,
              0,
            ),
            convenienceFee: asNumber(
              paymentDetails?.customerPayable?.convenienceFee,
              0,
            ),
            taxOnConvenience: asNumber(
              paymentDetails?.customerPayable?.taxOnConvenience,
              0,
            ),
            convenienceFeeBefore: asNumber(
              paymentDetails?.customerPayable?.convenienceFeeBefore,
              0,
            ),
            taxOnConvenienceBefore: asNumber(
              paymentDetails?.customerPayable?.taxOnConvenienceBefore,
              0,
            ),
            couponCode: paymentDetails?.customerPayable?.couponCode ?? null,
            discountAmount: asNumber(
              paymentDetails?.customerPayable?.discountAmount,
              0,
            ),
          },
          vendorReceivable: {
            ...
            baseAmount: asNumber(
              paymentDetails?.vendorReceivable?.baseAmount,
              0,
            ),
            commission: asNumber(
              paymentDetails?.vendorReceivable?.commission,
              0,
            ),
            taxOnCommission: asNumber(
              paymentDetails?.vendorReceivable?.taxOnCommission,
              0,
            ),
          },
        }
    ...
    const normalizedMethodDetails = Array.isArray(payment_method_details)
      ? payment_method_details.map((m) => ({
          ...
          payment_completion_time: m?.payment_completion_time
            ? new Date(m.payment_completion_time)
            : undefined,
    ...
    const items = Array.isArray(final_order_items)
      ? final_order_items.map((it) => ({
          ...
          service_asset: Array.isArray(it?.service_asset)
            ? it.service_asset
            : [],
    ...
    const doc = {
      ...
      vendor_name:
        req.body.vendor_name || vendor_segments?.[0]?.vendor_name || null,
      vendor_manager_name,
      customer_name,
      vendor_manager_contact_number,
      vendor_manager_contact_email,
    ...
    if (em_id && service_id) {
      const orderUpdatePayload = {
        ...
        paymentDetails: {
          paymentStatus:
            payStatusNorm === "fully_paid" ? "Fully Paid" : "Partially Paid",
          customerPayable: {
            total: asNumber(paymentDetails?.customerPayable?.total, 0),
            baseAmount: asNumber(
              paymentDetails?.customerPayable?.baseAmount,
      ...
      if (
        payStatusNorm === "fully_paid" ||
        (paymentDetails?.customerPayable?.total || 0) === 0
      ) {
        try {
          ...
          const vSegs =
            req.body.vendor_segments && req.body.vendor_segments.length > 0
          ...
          const sqsMessage = {
            ...
            customer: {
              ...
              mobile: doc.customer_contact_number,
              email: doc.customer_contact_email,
            },
            ...
            paymentDetails: {
              ...
              finalAmount:
                orderUpdatePayload.paymentDetails.customerPayable.total,
          ...
          console.log(
            `[BOOKING] Recorded FREE transaction and sent SQS for invoicing for Order: ${quotation_id}`,
          );
        } catch (syncErr) {
          console.error(
            "[BOOKING] Failed to sync free booking transaction/invoice:",
            syncErr.message,
          );
        }
      }
    ...
    try {
      if (normalizedMethodDetails && normalizedMethodDetails.length > 0) {
        ...
        const recentPayment =
          normalizedMethodDetails[normalizedMethodDetails.length - 1];
        if (recentPayment && recentPayment.payment_status === "SUCCESS") {
          ...
          if (breakdownNamePassed) {
            console.log(
              `[BOOKING] Attempting to mark payment breakdown "${breakdownNamePassed}" as Paid for ${effectiveQuotationId}`,
            );

            const updateQuery =
              breakdownNamePassed === "FULL"
                ? {
                    $set: {
            ...
            console.log(
              `[BOOKING] Mark Paid Result -> Orders: ${orderRes.modifiedCount}, Anon: ${anonRes.modifiedCount}, Events: ${eventRes.modifiedCount}`,
            );
          }
        }
      }
      ...
      console.error(
        "[BOOKING] Failed to map paymentBreakdowns paid status:",
        paymentBreakdownErr,
      );
    }
    ...
    try {
      if (Array.isArray(paymentBreakdowns) && paymentBreakdowns.length > 0) {
        ...
        for (const zb of zeroAmountPaid) {
          if (!zb.name) continue;
          console.log(
            `[BOOKING] Marking zero-amount breakdown "${zb.name}" as Paid on Order for ${effectiveQuotationId}`,
          );
      ...
      console.error(
        "[BOOKING] Failed to update zero-amount breakdown status:",
        zeroBreakdownErr,
      );
    }
    ...
    if (!effectiveQuotationId && event_id && event_id.startsWith("ODR")) {
      console.log(
        `[BOOKING] quotation_id missing. Attempting lookup via Order ID: ${event_id}`,
      );
      try {
        const sourceOrder = await Order.findOne({ order_id: event_id });
        if (sourceOrder && sourceOrder.quotation_id) {
          effectiveQuotationId = sourceOrder.quotation_id;
          console.log(
            `[BOOKING] Resolved quotation_id from Order: ${effectiveQuotationId}`,
          );
        }
      } catch (err) {
    ...
    console.log(
      `[BOOKING] Checking for anonymous order conversion. Effective Quotation ID: '${effectiveQuotationId}'`,
    );
    ...
    if (effectiveQuotationId && effectiveQuotationId.startsWith("ANON_ODR_")) {
      ...
      if (!anonOrder) {
        console.error(
          `[BOOKING] ❌ FATAL: AnonCustomerOrder NOT FOUND for ID: ${quotation_id}`,
        );
      } else {
      ...
      if (anonOrder) {
        ...
        console.log(
          `[BOOKING] Updated ${updateResult.modifiedCount} chat messages to converted status for ${effectiveQuotationId}`,
        );
        ...
        if (req.io && anonOrder.chat_id) {
          ...
          console.log(
            `[BOOKING] Emitted order_status_updated for ${effectiveQuotationId} to rooms: ${uniqueRooms.join(", ")}`,
          );
        }
      } else {
        console.warn(
          `[BOOKING] AnonCustomerOrder not found for conversion: ${effectiveQuotationId}`,
        );
      }
    }
...
export const addOfflineEvent = async (req, res) => {
  ...
  try {
    ...
    if (endDate < startDate) {
      return res
        .status(400)
        .json({ message: "end must be after or equal to start" });
    }
    ...
    return res
      .status(200)
      .json({ message: "Event added successfully", calendar });
  } catch (error) {
    console.error(error);
...
export const editOfflineEvent = async (req, res) => {
  try {
    ...
    if (!event_id || !updatedEventData) {
      return res.status(400).json({
        message:
          "Missing required fields: service_id, event_id, or updatedEventData",
      });
    }
...
export const getTransactionsByEventId = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ transactions, event_id, quotation_id: event.quotation_id });
  } catch (error) {
