...
export const createOrUpdateAnonOrder = async (req, res) => {
  try {
    ...
    console.log(
      `[ORDER_DEBUG] Request for anon_order_id: ${anon_order_id}, chat_id: ${chat_id}`,
    );
    console.log(
      `[ORDER_DEBUG] incoming advance_amount_requested: ${advance_amount_requested} (type: ${typeof advance_amount_requested})`,
    );
    ...
    console.log(
      `[ORDER] Fetching chat details for chat_id: '${trimmedChatId}'`,
    );
    const chat = await Chat.findOne({ chat_id: trimmedChatId });

    if (!chat) {
      console.log(
        `[ORDER] WARNING: Chat with id '${chat_id}' NOT FOUND in DB during order creation/update.`,
      );
    } else {
      console.log(
        `[ORDER] Chat found. Database chat_type: '${chat.chat_type}'`,
      );
    }

    const chatType = chat ? chat.chat_type : "anon_customer-admin";
    console.log(
      `[ORDER] Resolved chatType for Message creation: '${chatType}'`,
    );
    ...
    if (anon_order_id) {
      ...
      console.log(
        `[ORDER_DEBUG] existingOrder.advance_amount_requested: ${existingOrder.advance_amount_requested}`,
      );

      let paymentDetailsToUse =
        paymentDetails !== undefined
          ? paymentDetails
          : existingOrder.paymentDetails;
      let finalAmountToStore =
        final_amount !== undefined ? final_amount : existingOrder.final_amount;
      if (
        paymentDetailsToUse &&
        paymentDetailsToUse.customerPayable &&
        typeof paymentDetailsToUse.customerPayable.total === "number"
      ) {
        ...
        try {
          ...
          if (serviceModel) {
            ...
            if (serviceDoc && serviceDoc.basic_details) {
              vendor_manager_name =
                serviceDoc.basic_details.point_of_contact ||
                vendor_manager_name;
          ...
          vendor_manager_contact_email = "no-reply@eventory.com";
          console.error(
            "Failed to fetch service/vendor details for final order mapping:",
            err,
          );
        }
        const newOrder = await Orders.create({
          em_id:
            em_id !== undefined ? em_id : existingOrder.em_id || "EM_SYSTEM",
          service_id:
            service_id !== undefined
              ? service_id
              : existingOrder.service_id || "SERVICE_UNKNOWN",
          vendor_id:
            vendorId !== undefined
              ? vendorId
              : existingOrder.vendor_id || "VENDOR_UNKNOWN",
          quotation_id:
            anonOrderId || existingOrder.anon_order_id || "QUO_UNKNOWN",
          vendor_manager_name: vendor_manager_name || "Not Assigned",
          customer_name:
            customer_name !== undefined
              ? customer_name
              : existingOrder.customer_name || "Anonymous Customer",
          customer_id:
            anonUserId !== undefined
              ? anonUserId
              : existingOrder.anon_user_id || "CUSTOMER_UNKNOWN",
          event_start:
            event_start !== undefined
              ? event_start
              : existingOrder.event_start || new Date(),
          event_end:
            event_end !== undefined
              ? event_end
              : existingOrder.event_end ||
                new Date(Date.now() + 2 * 60 * 60 * 1000),
          event_type:
            event_type !== undefined
              ? event_type
              : existingOrder.event_type || "General Event",
          final_guest_count:
            guest_count !== undefined
              ? guest_count
              : existingOrder.guest_count || 0,
          location_type:
            location_type !== undefined
              ? location_type
              : existingOrder.location_type || "outdoor",
          event_location:
            event_location !== undefined
              ? event_location
              : existingOrder.event_location || "Location Not Provided",
          final_amount:
            finalAmountToStore !== undefined ? finalAmountToStore : 0,
          advance_amount_requested:
            advance_amount_requested !== undefined
              ? advance_amount_requested
              : existingOrder.advance_amount_requested || 0,
          paymentBreakdowns:
            paymentBreakdowns !== undefined
              ? paymentBreakdowns
              : existingOrder.paymentBreakdowns || [],
          vendor_approval: true,
          customer_approval: true,
          original_ask_by_customer:
            customer_requirements !== undefined
              ? customer_requirements
              : existingOrder.customer_requirements || "",
          order_status: "approved",
          vendor_manager_contact_number:
            vendor_manager_contact_number || "0000000000",
          vendor_manager_contact_email:
            vendor_manager_contact_email || "no-reply@eventory.com",
          customer_contact_number:
            customer_contact_number !== undefined
              ? customer_contact_number
              : existingOrder.customer_contact_number || "",
          customer_contact_email:
            customer_contact_email !== undefined
              ? customer_contact_email
              : existingOrder.customer_contact_email || "no-reply@eventory.com",
          final_order_items:
            final_order_items !== undefined
              ? final_order_items
              : existingOrder.final_order_items || [],
          paymentDetails: paymentDetailsToUse || {},
          specificTerms:
            specificTerms !== undefined
              ? specificTerms
              : existingOrder.specificTerms || [],
        });
        console.log(
          `[ORDER_DEBUG] Orders.create payload advance_amount_requested: ${newOrder.advance_amount_requested}`,
        );
      ...
      updatedFields = {
        ...updatedFields,
        service_id:
          service_id !== undefined ? service_id : existingOrder.service_id,
        vendor_id:
          vendor_id !== undefined ? vendor_id : existingOrder.vendor_id,
        vendor_name:
          vendor_name !== undefined ? vendor_name : existingOrder.vendor_name,
        customer_name:
          customer_name !== undefined
            ? customer_name
        ...
        customer_contact_email:
          customer_contact_email !== undefined
            ? customer_contact_email
            : existingOrder.customer_contact_email,
        event_type:
          event_type !== undefined ? event_type : existingOrder.event_type,
        event_start:
          event_start !== undefined ? event_start : existingOrder.event_start,
        event_end:
          event_end !== undefined ? event_end : existingOrder.event_end,
        event_time:
          event_time !== undefined ? event_time : existingOrder.event_time,
        event_location:
          event_location !== undefined
            ? event_location
            : existingOrder.event_location,
        location_type:
          location_type !== undefined
            ? location_type
            : existingOrder.location_type,
        guest_count:
          guest_count !== undefined ? guest_count : existingOrder.guest_count,
        ...
        advance_amount_requested:
          advance_amount_requested !== undefined
            ? advance_amount_requested
            : existingOrder.advance_amount_requested,
        paymentBreakdowns:
          paymentBreakdowns !== undefined
            ? paymentBreakdowns
            : existingOrder.paymentBreakdowns,
      ...
      console.log(
        `[ORDER_DEBUG] final advance_amount_requested to save (AnonCustomerOrder): ${updatedFields.advance_amount_requested}`,
      );
      ...
      try {
        ...
        console.log(
          `[ORDER] Order card message created: ${orderCardMessage._id}`,
        );
        ...
        if (req.io) {
          ...
          console.log(
            `[ORDER] Emitting socket events to rooms: ${uniqueRooms.join(", ")}`,
          );
        ...
        try {
          if (!order.anon_user_id) {
            console.error(
              "❌ Failed to create notification: anon_user_id is missing in anon order",
            );
          } else {
            console.log(
              `📝 Attempting to create persistent notification for customer ${order.anon_user_id} and order ${order.anon_order_id}`,
            );
            ...
            console.log(
              `✅ Persistent notification created/updated for anon order:`,
              notif._id,
            );
          }
        } catch (notifErr) {
          console.error(
            "❌ Failed to create persistent customer notification for anon order:",
            notifErr.message,
          );
        }
      } catch (msgError) {
      ...
      try {
        ...
        console.log(
          `[ORDER] System message created (New Order): ${systemMessage._id}`,
        );
        ...
        console.log(
          `[ORDER] Order card message created (New Order): ${orderCardMessage._id}`,
        );
        ...
        try {
          if (!order.anon_user_id) {
            console.error(
              "❌ Failed to create notification: anon_user_id is missing in new anon order",
            );
          } else {
            console.log(
              `📝 Attempting to create persistent notification for customer ${order.anon_user_id} and order ${order.anon_order_id}`,
            ...
            console.log(
              `✅ Persistent notification created/updated for NEW anon order:`,
              notif._id,
            );
          }
        } catch (notifErr) {
          console.error(
            "❌ Failed to create persistent customer notification for new anon order:",
            notifErr.message,
          );
        }
      } catch (msgError) {
