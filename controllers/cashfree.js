...
const NON_ONBOARDED_VENDOR_ID =
  process.env.IS_DEV === "true"
    ? "VEN04012026001126960"
    : "VEN05012026111140552";
const NON_ONBOARDED_SERVICE_ID =
  process.env.IS_DEV === "true"
    ? "CAT04012026002741768"
    : "VNP05012026115907886";
...
const createOrder = async (req, res) => {
  ...
  try {
    ...
    for (const item of serviceIdsToCheck) {
      ...
      if (serviceModel) {
        ...
        const bankDetailsValid = !!(
          serviceDoc?.bank_details &&
          (serviceDoc.bank_details.account_number ||
            serviceDoc.bank_details.upi_id)
        );
        if (!bankDetailsValid) {
    ...
    if (missingBankSegments.length > 0) {
      console.warn(
        `[createOrder] 🛑 Blocking order creation. Bank details missing for: ${missingBankSegments.map((s) => s.service_id).join(", ")}`,
      );
      ...
      try {
        ...
        if (em_id && final_chat_id) {
          const missingDetails = missingBankSegments
            .map((s) => `${s.vendor_name} (${s.service_id})`)
            .join(", ");
        ...
        console.error(
          "❌ [createOrder] Error creating EM notification:",
          notiErr,
        );
      }
...
export const verifyPayment = async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      console.log(
        `[VerifyVendorPayment] ✅ Successfully recorded PG_IN transaction for ${order_id}`,
      );
    } catch (txnErr) {
      console.error(
        `[VerifyVendorPayment] ❌ Failed to record PG_IN transaction:`,
        txnErr,
      );
    }
...
const verifyCustomerPayment = async (req, res) => {
  ...
  try {
    ...
    if (order_amount > 0) {
      ...
      if (payment.order_status !== "PAID") {
        console.warn(
          `[VerifyPayment] Payment status for ${order_id} is ${payment.order_status}. Notifying chat.`,
        );
        ...
        try {
          if (req.io) {
            ...
            const order_quotation_id = finalOrder
              ? finalOrder.quotation_id
              : internal_order_id;
            const chat = await Chat.findOne({
              $or: [
                { chat_id: chat_id },
                { chat_id: order_quotation_id },
                { quotation_id: order_quotation_id },
              ],
            });
            const finalChatId = chat_id || chat?.chat_id || order_quotation_id;
            const chatType = chat
              ? chat.chat_type
              : customer_id?.startsWith("ANON")
                ? "anon_customer-admin"
                : "customer-admin";
            ...
            console.log(
              `[VerifyPayment] Emitting FAILURE to rooms: ${uniqueRooms.join(", ")}`,
            );
          ...
          console.error(
            "❌ Failed to send failure notification to chat:",
            err.message,
          );
        }
      ...
      console.log(
        `[VerifyPayment] Zero amount payment detected for order ${internal_order_id}. Skipping Cashfree check.`,
      );
      payment = {
        order_status: "PAID",
    ...
    if (
      !effective_internal_order_id ||
      effective_internal_order_id === order_id
    ) {
      if (
        payment.order_tags &&
        payment.order_tags.internal_order_id &&
        payment.order_tags.internal_order_id !== "MISSING"
      ) {
        effective_internal_order_id = payment.order_tags.internal_order_id;
        console.log(
          `[VerifyPayment] Recovered internal_order_id from order_tags: ${effective_internal_order_id}`,
        );
      }
    }

    console.log(
      `[VerifyPayment] Searching for internal_order_id: ${effective_internal_order_id}`,
    );
    ...
    if (!finalOrder) {
      console.log(
        `[VerifyPayment] Order not found by order_id, trying quotation_id: ${effective_internal_order_id}`,
      );
      finalOrder = await Order.findOne({
        quotation_id: effective_internal_order_id,
      }).lean();
      if (!finalOrder) {
        console.error(
          `[VerifyPayment] ⚠️ Final order not found for internal_order_id: ${effective_internal_order_id}. BUT payment is PAID. Returning SUCCESS to frontend for recovery.`,
        );
    ...
    console.log(
      `[VerifyPayment] Found Order: ${finalOrder.order_id}, customer_id: ${finalOrder.customer_id}`,
    );
    ...
    if (finalCustomerId && finalCustomerId.startsWith("ANON")) {
      ...
      try {
        const anonUser = await AnonymousUser.findOne({
          anon_id: finalCustomerId,
        });
        if (anonUser) {
          console.log(
            "[VerifyPayment] Found anonymous user record. converted_user_id:",
            anonUser.converted_user_id,
          );
          if (anonUser.converted_user_id) {
            console.log(
              "[VerifyPayment] Resolved to customer ID:",
              anonUser.converted_user_id,
            );
            ...
            console.log(
              "[VerifyPayment] Updated final order customer ID:",
              finalCustomerId,
            );
          } else {
            console.log(
              "[VerifyPayment] Anonymous user NOT YET converted. Proceeding with anon ID.",
            );
          }
        } else {
          console.log(
            "[VerifyPayment] AnonymousUser record NOT FOUND for ID:",
            finalCustomerId,
          );
        }
      } catch (err) {
      ...
      console.log(
        `[VerifyPayment] Authenticated user mismatch. Updating order ${finalOrder.order_id} to customer_id ${customer_id}`,
      );
      await Order.findOneAndUpdate(
        { order_id: finalOrder.order_id },
    ...
    const customerEmail =
      customerDoc?.email_address ||
      customerDoc?.email ||
      finalOrder?.customer_email ||
      payment?.customer_details?.customer_email ||
      "";
    const customerPhone =
      customerDoc?.mobile_number ||
      customerDoc?.customer_contact_number ||
      finalOrder?.customer_contact_number ||
    ...
    const serviceDoc = ServiceModel
      ? await ServiceModel.findOne({ service_id: service_id }).lean()
      : null;

    // NEW: Resolve Event ID early so we can attach it to transactions immediately!
    let event_id;
    let existingEventForEarlyId = await Events.findOne({
        ...
        await Transaction.create({
          ...
          paymentDetails: {
            customerPayable: {
              ...
              baseAmount:
                finalOrder?.paymentDetails?.customerPayable?.baseAmount ?? 0,
              convenienceFee:
                finalOrder?.paymentDetails?.customerPayable?.convenienceFee ??
                0,
              taxOnConvenience:
                finalOrder?.paymentDetails?.customerPayable?.taxOnConvenience ??
                0,
            },
            vendorReceivable: {
              total: finalOrder?.paymentDetails?.vendorReceivable?.total ?? 0,
              baseAmount:
                finalOrder?.paymentDetails?.vendorReceivable?.baseAmount ?? 0,
              commission:
                finalOrder?.paymentDetails?.vendorReceivable?.commission ?? 0,
              taxOnCommission:
                finalOrder?.paymentDetails?.vendorReceivable?.taxOnCommission ??
                0,
            },
          },
        });
        console.log(
          `[VerifyPayment] ✅ Successfully recorded PG_IN transaction for ${order_id}`,
        );
      } catch (txnErr) {
        console.error(
          `[VerifyPayment] ❌ Failed to record PG_IN transaction:`,
          txnErr,
        );
      }
    } else {
      console.log(
        `[VerifyPayment] Skipped PG_IN transaction recording for zero amount payment.`,
      );
    }
    ...
    if (req.io) {
      try {
        ...
        const chatType = chat
          ? chat.chat_type
          : finalCustomerId?.startsWith("ANON")
            ? "anon_customer-admin"
            : "customer-admin";
        ...
        console.log(
          `[VerifyPayment] Emitting SUCCESS to rooms: ${uniqueRooms.join(", ")}`,
        );
        uniqueRooms.forEach((roomId) => {
          req.io.to(roomId).emit("new_message", savedMsg);
          req.io.to(roomId).emit("order_status_updated", {
            ...
            new_status:
              payment_type === "advance" ? "Partially Paid" : "Fully Paid",
          });
        });
      } catch (ioErr) {
        console.error(
          "❌ [VerifyPayment] Error emitting success notification:",
          ioErr,
        );
      }
    }
    ...
    const vendorSegments =
      finalOrder.vendor_segments && finalOrder.vendor_segments.length > 0
    ...
    console.log(
      `[VerifyPayment] Distributing payout across ${vendorSegments.length} segment(s) for milestone: ${payment_type}`,
    );
    ...
    if (selected_breakdowns && selected_breakdowns.length > 0) {
      console.log(
        `[VerifyPayment] Using selected_breakdowns for granular payout calculation.`,
      );
      for (const sb of selected_breakdowns) {
        ...
        const matchBreakdown = matchSeg?.paymentBreakdowns?.find(
          (b) => b.name === sb.name,
        );
        ...
        if (matchBreakdown) {
          const base = Number(
            matchBreakdown.vendor_base_amount || matchBreakdown.amount || 0,
          );
          const comm = Number(matchBreakdown.vendor_commission || 0);
          netBreakdownAmount = Math.max(0, base - comm);
        }

        selectedPayoutsMap.set(
          sid,
          selectedPayoutsMap.get(sid) + netBreakdownAmount,
        );
      }
    }
    ...
    for (const segment of vendorSegments) {
      ...
      const segAlreadyPaid = segPreviousPayouts.reduce(
        (sum, txn) => sum + (Number(txn.transfer_amount) || 0),
        0,
      );
      ...
      if (selected_breakdowns && selected_breakdowns.length > 0) {
        ...
        payment_type.toLowerCase().includes("last") ||
        payment_type.toLowerCase().includes("final") ||
        payment_type.toLowerCase().includes("late")
      ) {
        ...
        const milestone = (segment.paymentBreakdowns || []).find(
          (b) => b.name === payment_type,
        );
        if (milestone) {
          const netMilestone = Math.max(
            0,
            (Number(milestone.vendor_base_amount || milestone.amount) || 0) -
              (Number(milestone.vendor_commission) || 0),
          );
          // Backward compatibility fallback to milestone.amount if new fields are 0
          const calcAmount =
            netMilestone > 0 ? netMilestone : Number(milestone.amount) || 0;
      ...
      console.log(
        `[VerifyPayment] Segment ${segServiceId}: Receivable=${segReceivable}, AlreadyPaid=${segAlreadyPaid}, Share=${segPayoutAmount}`,
      );
      ...
      const segServiceDoc = SegServiceModel
        ? await SegServiceModel.findOne({ service_id: segServiceId })
        : null;
      if (!segServiceDoc) {
        console.error(
          `[VerifyPayment] Segment service ${segServiceId} not found. Skipping segment payout.`,
        );
        continue;
      }
      ...
      const segBeneficiaryId =
        segBank.beneficiary_id || generateUniqueId("BENE");
      if (!segBank.beneficiary_id && SegServiceModel) {
        await SegServiceModel.updateOne(
          { service_id: segServiceId },
          { $set: { "bank_details.beneficiary_id": segBeneficiaryId } },
        );
      }
      ...
      if (segVendorId !== NON_ONBOARDED_VENDOR_ID) {
        let exists = false;
        try {
          await axios.get(`${pBase}/beneficiary`, {
            headers: pHeaders,
            params: { beneficiary_id: segBeneficiaryId },
          });
          exists = true;
        } catch (e) {
          if (e?.response?.status === 404) {
            try {
              const inst = segHasBank
                ? {
                    bank_account_number: segBank.account_number,
                    bank_ifsc: segBank.ifsc,
                  }
                : { vpa: segBank.upi_id };
              await axios.post(
                `${pBase}/beneficiary`,
                {
                  ...
                  beneficiary_name:
                    segServiceDoc.business_details
                      ?.business_registration_name || segVendorName,
                  beneficiary_instrument_details: inst,
                  beneficiary_contact_details: {
                    beneficiary_email:
                      segServiceDoc.business_details?.business_email ||
                      "noreply@eventory.in",
                    beneficiary_phone: (
                      segServiceDoc.basic_details?.service_contact_number ||
                      "0000000000"
                    )
                      .replace(/\D/g, "")
                      .slice(-10),
              ...
              console.error(
                `[VerifyPayment] Failed to create beneficiary for ${segServiceId}:`,
                beneErr?.response?.data || beneErr.message,
              );
            }
          }
        }
      ...
      if (segBankValid && segVendorId !== NON_ONBOARDED_VENDOR_ID) {
        try {
          ...
          console.log(
            `[VerifyPayment] Payout initiated for ${segServiceId}: ${segPayoutAmount}`,
          );
        } catch (tErr) {
          console.error(
            `[VerifyPayment] Transfer failure for ${segServiceId}:`,
            tErr?.response?.data || tErr.message,
          );
          await Transaction.findOneAndUpdate(
            { transfer_id: segTransferId },
    ...
    const receivableFromOrder = Number(
      finalOrder?.paymentDetails?.vendorReceivable?.total || 0,
    );
    ...
    console.log(
      `[VerifyPayment] Updating order ${internalOrderId} with transactionId: ${order_id}`,
    );
    ...
    console.log(
      `[VerifyPayment] Order update result. transactionId in DB: ${updatedOrder?.paymentDetails?.transactionId}`,
    );

    // Update matching paymentBreakdown status to "Paid" (including segments)
    if (selected_breakdowns && selected_breakdowns.length > 0) {
      console.log(
        `[VerifyPayment] Updating granular breakdowns based on selected_breakdowns array.`,
      );
      const orderDocForSave = await Order.findOne({
        order_id: internalOrderId,
      });
      if (orderDocForSave && orderDocForSave.vendor_segments) {
        for (const sb of selected_breakdowns) {
          const seg = orderDocForSave.vendor_segments.find(
            (s) => s.service_id === sb.service_id,
          );
        ...
        if (orderDocForSave.paymentBreakdowns) {
          for (const sb of selected_breakdowns) {
            const bk = orderDocForSave.paymentBreakdowns.find(
              (b) => b.name === sb.name,
            );
      ...
      console.log(
        `[VerifyPayment] Marked breakdown "${payment_type}" as Paid for order ${internalOrderId} and all segments`,
      );
    ...
    try {
      if (req.io) {
        console.log(
          `📡 Emitting payment success message to chat room ${quotation_id}`,
        );
        ...
        console.warn(
          "⚠️ req.io is missing, cannot emit payment success message",
        );
      }
    } catch (msgErr) {
      console.error(
        "❌ Failed to send real-time payment notification to chat:",
        msgErr.message,
      );
    }
    ...
    sendFCMNotificationToEm({
      ...
      .then((result) => {
        console.log(
          `FCM notifications sent to em ${em_id} for payment ${order_id}`,
          result,
        );
      })
      .catch((error) => {
    ...
    sendFCMNotificationToVendor({
      ...
      .then((result) => {
        console.log(
          `FCM notifications sent to vendor ${vendor_id} for payment ${order_id}`,
          result,
        );
      })
      .catch((error) => {
    ...
    if (existingEvent) {
      ...
      console.log(
        `[VerifyPayment] Existing event found: ${event_id}. Updating payment amounts...`,
      );

      const currentPaid = Number(existingEvent.already_paid_amount || 0);
      const newTotalPaid = Number(
        (currentPaid + Number(order_amount)).toFixed(2),
      );
      ...
      const updateData = {
        $set: {
          ...
          payment_status:
            newTotalPaid >= (existingEvent.final_amount || 0) ||
            payment_type.toLowerCase().includes("late") ||
            payment_type === "remaining"
              ? "fully_paid"
              : "advance_paid",
          payment_details: {
            customerPayable: {
              ...
              couponCode:
                couponCode ||
                existingEvent.payment_details?.customerPayable?.couponCode ||
                null,
              couponDiscount:
                N(couponDiscount) ||
                N(
              ...
              discountAmount:
                N(couponDiscount) + N(breakdownDiscount) ||
                N(
                  existingEvent.payment_details?.customerPayable
                    ?.discountAmount || 0,
                ),
      ...
      if (payment_type === "remaining" || payment_type === "full") {
        ...
        updateData.$set[
          "vendor_segments.$[seg].paymentBreakdowns.$[b].status"
        ] = "Paid";
        updateData.$set[
          "vendor_segments.$[seg].paymentBreakdowns.$[b].paid_at"
        ] = new Date();
        ...
        updateData.$set[
          "vendor_segments.$[seg].paymentBreakdowns.$[elem].status"
        ] = "Paid";
        updateData.$set[
          "vendor_segments.$[seg].paymentBreakdowns.$[elem].paid_at"
        ] = new Date();
        updateOptions.arrayFilters = [
      ...
      await Events.updateMany(
        ...
        },
        {
          arrayFilters: [
            {
              "elem.name": { $regex: /token/i },
              "elem.amount": { $in: [0, "0", null] },
            },
          ],
        },
      );
      console.log(
        `[VerifyPayment] Event ${event_id} updated. New total paid: ${newTotalPaid}`,
      );
    } else {
      ...
      console.log(
        `[VerifyPayment] No existing event found. Creating NEW event ${event_id} for Customer ID: ${finalCustomerId} | Order ID: ${internalOrderId}`,
      );
      const preBooking = new Events({
        ...
        final_guest_count: Math.max(
          1,
          Number(finalOrder?.final_guest_count || 1),
        ),
        ...
        vendor_manager_contact_number: isValidINMobile(
          serviceDoc?.basic_details?.service_contact_number,
        )
          ? serviceDoc?.basic_details?.service_contact_number
          : "",
        ...
        payment_details: {
          customerPayable: {
            ...
            baseAmount: N(
              finalOrder?.paymentDetails?.customerPayable?.baseAmount,
            ),
            convenienceFee: N(
              finalOrder?.paymentDetails?.customerPayable?.convenienceFee,
            ),
            ...
            convenienceFeeBefore: N(
              finalOrder?.paymentDetails?.customerPayable?.convenienceFeeBefore,
            ),
            taxOnConvenienceBefore: N(
              finalOrder?.paymentDetails?.customerPayable
                ?.taxOnConvenienceBefore,
            ),
            couponCode:
              couponCode ||
              finalOrder?.paymentDetails?.customerPayable?.couponCode ||
              null,
            couponDiscount:
              N(couponDiscount) ||
              N(finalOrder?.paymentDetails?.customerPayable?.couponDiscount),
            breakdownDiscount:
              N(breakdownDiscount) ||
              N(finalOrder?.paymentDetails?.customerPayable?.breakdownDiscount),
            discountAmount:
              N(couponDiscount) + N(breakdownDiscount) ||
              N(finalOrder?.paymentDetails?.customerPayable?.discountAmount),
          },
          vendorReceivable: {
            total: N(finalOrder?.paymentDetails?.vendorReceivable?.total),
            baseAmount: N(
              finalOrder?.paymentDetails?.vendorReceivable?.baseAmount,
            ),
            commission: N(
              finalOrder?.paymentDetails?.vendorReceivable?.commission,
            ),
            taxOnCommission: N(
              finalOrder?.paymentDetails?.vendorReceivable?.taxOnCommission,
            ),
      ...
      console.log(
        `[VerifyPayment] Order ${internalOrderId} updated with event_id: ${event_id}`,
      );

      // Send Slack notification for new booking
      const { sendSlackBookingMessage } = await import(
      ...
      if (process.env.IS_DEV !== "true") {
        sendSlackBookingMessage({
          ...
          startDate: new Date(preBooking.event_start).toLocaleDateString(
            "en-IN",
            {
              year: "numeric",
              month: "long",
    ...
    const discountAbs =
      typeof couponDiscount === "number"
        ...
        : Number(
            finalOrder?.paymentDetails?.customerPayable?.discountAmount ||
              finalOrder?.paymentDetails?.discount ||
              0,
          );
    ...
    const contents = Array.isArray(finalOrder?.final_order_items)
      ? finalOrder.final_order_items
      : [];
    ...
    const safeAlreadyPaid = existingEvent
      ? Number(existingEvent.already_paid_amount || 0)
      : 0;
    const paidAmount =
    ...
    const vendorPayload = {
      ...
      businessDetails: {
        businessName:
          serviceDoc?.business_details?.business_registration_name || "",
    ...
    const paymentDetailsMsg = {
      ...
      convinienceFee: String(
        Number((convenienceFee + taxOnConvenience).toFixed(2)),
      ),
    ...
    console.log(
      `[VerifyPayment] Sending SQS message for invoicing:`,
    ...
    try {
      const allBreakdownsPaid =
        updatedEvent?.payment_breakdowns?.length > 0 &&
        updatedEvent.payment_breakdowns.every(
          (b) => b.status === "Paid" || Number(b.amount || 0) === 0,
        );

      if (allBreakdownsPaid) {
        console.log(
          `[VerifyPayment] All breakdowns paid for ${event_id}. Sending consolidated invoice SQS.`,
        );
        ...
        console.log(
          `[VerifyPayment] Consolidated invoice SQS sent for ${event_id}.`,
        );
      }
    } catch (consolidatedErr) {
      console.error(
        `[VerifyPayment] Failed to send consolidated invoice SQS:`,
        consolidatedErr.message,
      );
    }
    return res.status(200).json({
    ...
    console.error(
      "❌ verifyCustomerPayment UNEXPECTED ERROR:",
