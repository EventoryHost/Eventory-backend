...
export const markAsReadAndDelete = async (req, res) => {
  ...
  if (
    ...
    !["vendor", "customer", "em"].includes(type)
  ) {
    return res
      .status(400)
      .json({ message: "Invalid Notification ID or type." });
  }
  ...
  try {
    ...
    if (!isUnreadCounter) {
      console.log(
        `[API] ⛔ Not deleting (not a worker-generated unread notification). ID: ${notificationId}`,
      );
