...
export const handleInteractiveMessage = async (
  ...
  try {
    ...
    const cleanSocketSenderId = (socketSenderId || "")
      .toString()
      .replace(/['"]/g, "")
      .trim();
    ...
    if (!userId) {
      console.warn(
        `[CHAT_INTERACTIVE] No userId found for chat: ${cleanChatId}`,
      );
      return;
    }
    ...
    let enquiry = null;
    try {
      enquiry = await CustomerEnquiry.findOne(enquiryQuery).sort({
        created_at: -1,
      });
    } catch (dbErr) {
      console.error(
        `[CHAT_INTERACTIVE] Database error finding enquiry:`,
        dbErr,
      );
    }
      ...
      return new Promise((resolve) => {
        setTimeout(async () => {
          try {
            ...
            if (!chat) {
              console.error(
                `[CHAT_SERVICE] Chat not found for ID during delayed send: ${chatId}`,
              );
              resolve(null);
              return;
            }
            ...
            console.log(
              `[CHAT_SERVICE] Emitting message to rooms: ${roomAnon}, ${roomCust}`,
            );
            if (io) {
            ...
            console.error(
              `[CHAT_SERVICE] Error sending message to participants:`,
              error,
            );
            resolve(null);
          }
    ...
    if (!enquiry || enquiry.status === "OPEN" || isKnownEventType) {
      console.log(
        `[CHAT_SERVICE] Step 2: Handling Event Type selection. Match: ${isKnownEventType ? "keyword" : "custom string"}`,
      );
      ...
      triggerSlackListAndWebhookNotification(enquiry, "create").catch((err) => {
        console.error(
          "[SLACK_INTEGRATION_TRIGGER] Initial Slack ticket creation failed:",
          err,
        );
      });

      await sendMessage(
        "Love it! Now, when are you planning to host it?",
        "date_picker",
    ...
    if (!enquiry) {
      console.warn(
        `[CHAT_INTERACTIVE] No active enquiry found for user: ${userId}. Skipping. Content: "${normalizedContent}"`,
      );
      return;
    }
    ...
    if (enquiry.status === "COLLECTING_LOCATION") {
      ...
      await sendMessage(
        "Is this event happening at home or at an outside venue?",
        "options",
        [
          { label: "At home", value: "At home" },
          {
            label: "Outdoor / Outside venue",
            value: "Outdoor / Outside venue",
          },
        ],
      );
      return;
    ...
    if (enquiry.status === "COLLECTING_VENUE_SETTING") {
      ...
      if (
        lowerContent.includes("outside") ||
        lowerContent.includes("outdoor")
      ) {
        ...
        await syncEnquiryToChat();
        await sendMessage(
          "Perfect! What kind of services are you looking for? (Select all that apply)",
          "multi_select",
          [
    ...
    if (enquiry.status === "COLLECTING_SERVICES") {
      ...
      const hasOthers = services.some((s) =>
        s.toLowerCase().includes("others"),
      );
      ...
      if (hasOthers) {
        ...
        await syncEnquiryToChat();
        await sendMessage(
          "Could you tell us more about the other services you need?",
        );
        return;
      }
      ...
      if (needsGuests) {
        ...
        await syncEnquiryToChat();
        await sendMessage(
          "Around how many guests are you expecting?",
          "options",
          [
        ...
        await syncEnquiryToChat();
        await sendMessage(
          "Almost there! Do you have a budget in mind for this event?",
          "options",
          [
    ...
    if (enquiry.status === "COLLECTING_BUDGET_OPTION") {
      ...
      if (hasBudget) {
        ...
        await sendMessage("What's your approximate budget range?", "options", [
          ...
          {
            label: "Prefer to discuss with the manager",
            value: "Prefer to discuss with the manager",
          },
        ]);
      } else {
    ...
    if (enquiry.status === "COLLECTING_PHONE") {
      ...
      await syncEnquiryToChat();
      await sendMessage(
        "Best time to call? (Optional but helpful)",
        "options",
        [
    ...
    if (enquiry.status === "COLLECTING_CALL_TIME") {
      ...
      await sendMessage(
        `Wonderful, ${name}! Your event brief is with us. One of our Event Managers will call you shortly at ${phone}.`,
        "text",
      );

      await sendMessage(
        "While you wait - here's what our clients say about us.",
        "options",
        [
      ...
      triggerSlackListAndWebhookNotification(enquiry, "complete").catch(
        (err) => {
          console.error(
            "[SLACK_INTEGRATION_TRIGGER] Final Slack ticket update failed:",
            err,
          );
        },
      );
...
const handleStep8Handoff = async (enquiry, sendMessage, syncEnquiryToChat) => {
  ...
  triggerSlackListAndWebhookNotification(enquiry, "update_requirements").catch(
    (err) => {
      console.error(
        "[SLACK_INTEGRATION_TRIGGER] Slack ticket requirements update failed:",
        err,
      );
    },
  );
  ...
  await sendMessage(
    "This already sounds exciting! Here’s how we’ll help you:",
    "text",
    null,
    null,
    50,
...
So you can enjoy the event stress-free! To make sure they can reach you quickly, could you share a couple of details?`;
  ...
  await sendMessage(
    "Your name? (First name works just fine!)",
    "text",
    null,
    null,
    1000,
...
const triggerCustomerNotification = async (
  customerId,
  chatId,
  messageContent,
  io,
