...
invoicesSchema.pre("save", async function (next) {
  try {
    ...
    if (this.type === "registration" && this.customer_id) {
      return next(
        new Error("Customer ID should be null for registration type invoices"),
      );
    }

    if (this.type !== "registration" && !this.customer_id) {
      return next(
        new Error("Customer ID is required for non-registration invoices"),
      );
    }
    ...
    if (this.type === "registration" && this.event_id) {
      return next(
        new Error("Event ID should be null for registration type invoices"),
      );
    }
