...
const createPhotographer = async (req, res) => {
  try {
    ...
    const newPAV = new PhotographerVideographer({
      ...
      additional_details: {
        asset_images: Array.isArray(asset_images)
          ? asset_images
          : [asset_images],
    ...
    const fieldsToCheck = [
      ...
      newPAV.basic_details.min_booking_capacity,
      newPAV.basic_details.max_booking_capacity,
...
const getAllPav = async (req, res) => {
  try {
    ...
    const pav = await PhotographerVideographer.find()
      .skip(skip)
      .limit(itemsPerPage);

    const totalpav = await PhotographerVideographer.countDocuments();
